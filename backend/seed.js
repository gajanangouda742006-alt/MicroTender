/**
 * Database Seeder - MySQL async version
...
async function seed() {
  ...
  await db.run('DELETE FROM fraud_logs', []);
  await db.run('DELETE FROM ratings', []);
  await db.run('DELETE FROM applications', []);
  await db.run('DELETE FROM micro_tenders', []);
  ...
  await db.run('ALTER TABLE users AUTO_INCREMENT = 1', []);
  await db.run('ALTER TABLE complaints AUTO_INCREMENT = 1', []);
  await db.run('ALTER TABLE micro_tenders AUTO_INCREMENT = 1', []);
  await db.run('ALTER TABLE vendors AUTO_INCREMENT = 1', []);
  await db.run('ALTER TABLE applications AUTO_INCREMENT = 1', []);
  await db.run('ALTER TABLE ratings AUTO_INCREMENT = 1', []);
  ...
  for (const u of users) {
    await db.run(
      ...
      [u.name, u.email, u.phone, u.password, u.role, u.govt_id_type, u.govt_id_number]
    );
  }
  ...
  for (const v of vendorProfiles) {
    await db.run(
      ...
      [v.userId, v.company, v.category, v.skills, v.lat, v.lon, v.address, v.exp, v.rating, v.jobs]
    );
  }
  ...
  for (const c of complaints) {
    await db.run(
      ...
      [c.userId, c.category, c.desc, c.lat, c.lon, c.status]
    );
  }
  ...
  for (const t of tenders) {
    await db.run(
      ...
      [t.complaintId, t.cost, t.priority, t.status, t.vendorId]
    );
  }
  ...
  for (const a of applications) {
    await db.run(
      ...
      [a.tenderId, a.vendorId, a.bid, a.proposal, a.status]
    );
  }
  ...
  await db.run(
    ...
    [1, 1, 2, 5, 'Excellent work! Pothole fixed perfectly.']
  );
